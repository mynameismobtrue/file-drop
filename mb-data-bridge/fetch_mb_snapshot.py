...
def get_json(url, attempts=3):
    ...
    for i in range(attempts):
        ...
        try:
            ...
            last_error = f"{type(e).__name__}:{e}"[:240]
...
def normalize_ticker(raw):
    ...
    if not isinstance(x, dict):
        return None
    return {
        "pair": str(x.get("pair") or x.get("symbol") or "BTC-BRL"),
...
def validate_book(b):
    ...
    if not b["bids"]:
        return False, "BIDS_EMPTY"
    if not b["asks"]:
        return False, "ASKS_EMPTY"
    if not b["best_bid"] or not b["best_ask"]:
        return False, "BEST_PRICE_INVALID"
    if b["best_ask"] <= b["best_bid"]:
        return False, "ASK_NOT_GREATER_THAN_BID"
    return True, None
...
def vwap_buy(asks, brl, best_ask):
    ...
    for price, qty in asks:
        ...
        btc += take / price
        remaining -= take
    if remaining > 1e-6 or btc <= 0:
        return {"amount_brl": brl, "sufficient_depth": False, "vwap_brl": None, "slippage_pct_vs_best_ask": None, "btc_acquired": None}
    vwap = spent / btc
    slip = ((vwap / best_ask) - 1) * 100 if best_ask else None
...
def main():
    fetched_at = now_iso()
    ticker_raw = get_json(TICKER_URL, attempts=2)
    book_raw = get_json(BOOK_URL, attempts=3)
    ...
    (OUT_DIR / "snapshot.json").write_text(json.dumps(snapshot, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
